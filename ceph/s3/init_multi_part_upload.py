...
import datetime
import sys
import xml.dom.minidom as dom # for xml parsing

if len(sys.argv) < 4:
    print('bad syntax, usage: {script_name} host bname oname')
    exit()

host, bkt, obj = sys.argv[1], sys.argv[2], sys.argv[3]
...
req = Request('http://' + host + '/' + bkt + '/' + obj + '?uploads',
            method = 'POST')
...
with urllib.request.urlopen(req) as f:
    uploadId = 'error'
    if f.status == 200:
        d = dom.parseString(f.read().decode('utf-8'))
        uploadId = d.getElementsByTagName('UploadId')[0].childNodes[0].data
    print(str(f.status) + ':' + uploadId)
